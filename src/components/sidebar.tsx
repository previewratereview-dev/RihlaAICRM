...
interface SidebarItem {
  ...
  section?: 'Sales' | 'Operations' | 'Communication' | 'Administration';
}
...
export function Sidebar() {
  ...
  const filteredSidebarItems = sidebarItems.filter((item) => {
    ...
    if (role === 'viewer') {
      return ['dashboard', 'leads', 'inquiries', 'analytics'].includes(item.id);
    }

    // Specialist / setter: operational tabs only
    if (role === 'specialist' || role === 'setter') {
      return ['dashboard', 'leads', 'inquiries', 'conversations', 'calendar', 'tasks'].includes(item.id);
    }

    // Consultant: same as specialist but with analytics
    if (role === 'consultant') {
      return ['dashboard', 'leads', 'inquiries', 'conversations', 'calendar', 'tasks', 'analytics'].includes(item.id);
    }
  ...
  const itemsBySection = filteredSidebarItems.reduce(
    (acc, item) => {
    ...
    >
        ...
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          {sidebarExpanded && (
            ...
            >
              ...
              <span className="text-[8px] text-muted-foreground font-mono mt-0.5 tracking-wider uppercase">
                Travel CRM
              </span>
            </motion.div>
          )}
      ...
      <nav className="flex-1 space-y-4 px-3 py-4 overflow-y-auto scrollbar-thin" aria-label="Main navigation">
        ...
        {itemsBySection['default'] && (
          <div className="space-y-1">{itemsBySection['default'].map(renderSidebarItem)}</div>
        )}
          ...
          return (
            <div key={section} className="flex flex-col gap-1">
              ...
              {(!isCollapsed || !sidebarExpanded) && (
                <div className="space-y-1">{items.map(renderSidebarItem)}</div>
              )}
            </div>
          );
        })}
